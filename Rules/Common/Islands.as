...
void onInit( CRules@ this )
{
	...
	Block::queryWeights( this );
	Block::queryCosts( this );
}
	...
	for (uint i = 0; i < islands.length; ++i)
	{
		...
		{			
			...
			if (b !is null)
			{
				...
				if ( b.hasTag( "seat" ) && b.getTickSinceCreated() > mostTicks )
				{
					string seatOwner = b.get_string( "playerOwner" );
					if ( seatOwner != "" )//Seat.as makes sure player is connected
					{
						mostTicks = b.getTickSinceCreated();
...
void InitIsland( Island @isle )
{
	...
	isle.mass = Maths::Pow( totalMass, 1.07f );//exponential mass growth
...
void onCommand( CRules@ this, u8 cmd, CBitStream @params )
{
	if (getNet().isServer())
		return;
	//todo: make it sync island.owner
