...
{
	if (g_videorecording || !getNet().isClient())
		return;
	...
	for ( u8 i = 0; i < boardTeams.length; i++ )
		GUI::DrawText( this.getTeam( boardTeams[i] ).getName(), panelStart + Vec2f( 0, (i+1)*lineHeight ), getTeamColor( boardTeams[i] ) );
