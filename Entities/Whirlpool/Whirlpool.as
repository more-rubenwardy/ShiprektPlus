...
void damageBlobs( CBlob@ this )
{
	...
	if ( getMap().getBlobsInRadius( this.getPosition(), 50.0f, @nearby ) )
	{
		...
		{
			if ( ( nearby[i].getName() == "block" && XORRandom(2) == 0 ) || ( nearby[i].getName() == "human" && !nearby[i].get_bool( "onGround" ) ) )
				this.server_Hit( nearby[i], Vec2f_zero, Vec2f_zero, this.getInitialHealth()/4.0f, 0, true );
			
			if ( nearby[i] !is this && this.getDistanceTo( nearby[i] ) < 15.0f )
			{
				this.server_Hit( nearby[i], Vec2f_zero, Vec2f_zero, 999.0f, 0, true );
				nearby[i].server_Die();
			}
		}
	}
