...
void onInit( CBlob@ this )
{
	...
	this.set_u8( "overDriveCharge", 100 );
...
void onTick( CBlob@ this )
{
	...
	u8 charge = this.get_u8( "overDriveCharge" );
	if ( charge < 100 && gameTime % 20 == 0 )
		this.set_u8( "overDriveCharge", Maths::Min( 100, charge + 17 ) );
	...
	if (on)
	{
		...
		if (island !is null)
		{
			...
			if ( overDrive )
			{
				...
				if ( charge > 1 )
					this.set_u8( "overDriveCharge", Maths::Round( charge * 0.975f ) );
			}
			...
			// effects
			if ( getNet().isClient() && getTicksASecond() >= 15 && !isTouchingLand(pos) )
			{
				u8 tickStep = v_fastrender ? 10 : 2;
