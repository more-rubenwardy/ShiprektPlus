	...
	if (getMap().getHitInfosFromRay( pos, -vel.Angle(), vel.Length(), this, @hitInfos ))
	{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if ( !b.hasTag( "booty" ) && (color > 0 || !isBlock) )
			{
				...
				{
					if ( Block::isSolid(blockType) || ( b.getTeamNum() != this.getTeamNum() && ( blockType == Block::MOTHERSHIP5 || b.hasTag("weapon") || blockType == Block::BOMB ) ) )//hit these and die
						killed = true;
...
void onHitBlob( CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData )
{
	...
	else	if (Block::isSolid(blockType) || blockType == Block::MOTHERSHIP5 || hitBlob.hasTag("weapon") || blockType == Block::PLATFORM || blockType == Block::SEAT || Block::isBomb( blockType ))
	{
		//effects
		sparks(worldPoint, 8);
...
void damageBooty( CPlayer@ attacker, CBlob@ attackerBlob, CBlob@ victim )
{
	if ( victim.getName() == "block" )
	{
		...
		if ( victimIsle !is null && victimIsle.blocks.length > 3
			...
			&& ( victim.hasTag("propeller") || victim.hasTag("weapon") || blockType == Block::MOTHERSHIP5 || Block::isBomb( blockType ) || blockType == Block::SEAT )
			)
		{
