		...
		if (getMap().getHitInfosFromRay( pos, -vel.Angle(), vel.Length(), this, @hitInfos ))
		{
			...
			for (uint i = 0; i < hitInfos.length; i++)
			{
				...
				if (color > 0 || !isBlock)
				{
					...
					{
						if ( blockType == Block::MOTHERSHIP5 || Block::isSolid(blockType) ||  ( (b.hasTag( "weapon" ) || b.hasTag( "rocket" )) && !sameTeam ) )
							killed = true;
...
void damageBooty( CPlayer@ attacker, CBlob@ attackerBlob, CBlob@ victim )
{
	if ( victim.getName() == "block" )
	{
		...
		if ( victimIsle !is null && victimIsle.blocks.length > 3
			...
			&& ( Block::isSolid(blockType) || victim.hasTag("weapon") || blockType == Block::MOTHERSHIP5 || Block::isBomb( blockType ) || blockType == Block::SEAT )
			)
		{
