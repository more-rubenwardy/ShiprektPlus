...
shared class HoverMessage
{
    ...
    HoverMessage( string _name, int _quantity, SColor _color = color_white, uint _ttl = 75, uint _fade_ratio = 2, bool singularise = true ) 
	{
...
shared class HoverMessage2
{
    ...
    HoverMessage2() {} // required for handles to work

    HoverMessage2( string _name, int _quantity, SColor _color = color_white, uint _ttl = 75, uint _fade_ratio = 2, bool singularise = true, string _prefix = "") 
	{
        if (_quantity >= 0 &&_quantity < 2 && singularise) 
		{
            _name = this.singularize(_name);
