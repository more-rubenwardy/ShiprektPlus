...
void onTick(CRules@ this)
{
	...
	if ( totalB < players * TOTAL_AMMONT_FACTOR )
	{
		...
		{
			f32 xSpot = Maths::Min( float( XORRandom( mWidth ) ) + 8.0f * 5.0f, mWidth - 8.0f * 10.0f );
			f32 ySpot = Maths::Min( XORRandom( mHeight ) + 8.0f * 5.0f, mHeight - 8.0f * 10.0f );
...
{
	bool mothership = map.isBlobWithTagInRadius( "mothership", spot, 1.35f * CLEAR_RADIUS );
	bool player = map.isBlobWithTagInRadius( "player", spot, 1.35f * CLEAR_RADIUS );
	bool booty = map.isBlobWithTagInRadius( "booty", spot, CLEAR_RADIUS );
